...
export const useCachingFetch: UseCachingFetch = (url) => {
  const localCacheData = localStorageCache[url];

  const data = localCacheData ? localCacheData.data : null;
  const isLoading = localCacheData ? false : true;
  const error = localCacheData ? localCacheData.error : null;
  ...
  useEffect(() => {
    if (!localCacheData) {
      fetch(url)
        ...
        .then((data) => {
          localStorageCache[url] = { data, error: null };
          setFetchedData({ data, isLoading: false, error: null });
        })
        .catch((error) => {
          localStorageCache[url] = { data: null, error };
          setFetchedData({ data: null, isLoading: false, error });
        });
    }
  }, [url, localCacheData]);
...
export const preloadCachingFetch = async (url: string): Promise<void> => {
  if (!localStorageCache[url]) {
    try {
      const response = await fetch(url);
      const data = await response.json();
      localStorageCache[url] = { data, error: null };
    } catch (error) {
      localStorageCache[url] = {
        data: null,
...
export const serializeCache = (): string => {
  return JSON.stringify(localStorageCache);
};

export const initializeCache = (serializedCache: string): void => {
  localStorageCache = JSON.parse(serializedCache);
};
